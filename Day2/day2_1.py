    ...
    for range_nums in range(int(split_range[0]), int(split_range[1]) + 1):
        ...
        for factor in factors_of_perm[num_len - 1]:
            ...
            if check_for_pattern:
                ...
                break
# didn't realise that the pattern can only repeat just twice to be invalid
# this is a bit of a hack to correct it.
# this takes the ids from the array that repeat twice or more e.g. 111, sees if the digit
# length is divisible by 2 because odd lengths cant repeat just twice
# it then splits the id in half and compares the first half with the last half
# if its the same then its added to the array
actual_valid_ids = []
for num in valid_ids:
    num_str = str(num)
    num_len = len(num_str)
    num_len_is_even = num_len % 2
    if num_len_is_even == 0:
        half_num_len = num_len / 2
        int_half_num_len = int(half_num_len)
        num1 = num_str[0:int_half_num_len]
        num2 = num_str[int_half_num_len:num_len]
        print(f"firstnum: {num1}, secondnum: {num2}")
        if num1 == num2:
            actual_valid_ids.append(num)

print(f"valid id's: {valid_ids}")
print(f"sum of id's: {sum(valid_ids)}")

print(f"actual valid id's: {actual_valid_ids}")
print(f"sum of actual id's: {sum(actual_valid_ids)}")
